...
from micropython import const, heap_locked
...
_IRQ_BOND_READ = const(24)
_IRQ_BOND_WRITE = const(25)
_IRQ_BOND_DELETE = const(26)
...
SEC_FIELDS = ('bond_type', 'addr_type', 'addr', 'key_size', 'ediv', 'rand', 'ltk', 'irk', 'csrk', 'authenticated', 'secure_connection')
KEYSTORE = []
...
def irq(event, data):
    ...
    if event == _IRQ_CENTRAL_CONNECT:
    ...
    elif event == _IRQ_BOND_READ:
        global KEY_FIELDS
        print("_IRQ_BOND_READ")
        print(KEY_FIELDS)
        print(data)
        if heap_locked():
            return
        ...
        print("_IRQ_BOND_WRITE", data)
        details = zip(SEC_FIELDS, data)
        KEYSTORE.append(details)
        print(KEYSTORE)

    elif event == _IRQ_BOND_DELETE:
        print("_IRQ_BOND_DELETE", zip(KEY_FIELDS, data))
...
def instance1():
    ...
    try:
        for i in range(7):
            ...
            print("gap_pair")
            ble.gap_pair(conn_handle, bond=True, lesc=True)
